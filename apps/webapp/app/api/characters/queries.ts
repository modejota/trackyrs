import { useInfiniteQuery, useQuery } from "@tanstack/react-query";
import type { CharacterSearchEnvelope, CharacterWithRelations } from "./types";

const base = process.env.NEXT_PUBLIC_HONO_SERVER_URL as string;

export function useCharacterDetails(characterId: number) {
	return useQuery<CharacterWithRelations>({
		queryKey: ["character", characterId],
		queryFn: async () => {
			const res = await fetch(`${base}/api/characters/${characterId}`, {
				credentials: "include",
			});
			if (!res.ok) throw new Error(`Failed to fetch character: ${res.status}`);
			const json = await res.json();
			if (!json?.success || !json?.data)
				throw new Error("Invalid response shape");
			return json.data as CharacterWithRelations;
		},
		staleTime: 60 * 60 * 1000, // 1 hour
		gcTime: 5 * 60 * 60 * 1000, // 5 hours
	});
}

export function useInfiniteCharacterSearch(name: string, limit = 24) {
	const base = process.env.NEXT_PUBLIC_HONO_SERVER_URL as string;
	return useInfiniteQuery({
		queryKey: ["characters", "search", name, limit],
		queryFn: async ({ pageParam = 1 }) => {
			const params = new URLSearchParams();
			params.append("page", String(pageParam ?? 1));
			params.append("limit", String(limit));
			if (name) params.append("name", name);
			const res = await fetch(
				`${base}/api/characters/search?${params.toString()}`,
				{
					credentials: "include",
				},
			);
			if (!res.ok)
				throw new Error(`Failed to search characters: ${res.status}`);
			const json: CharacterSearchEnvelope = await res.json();
			if (!json?.success || !json?.data)
				throw new Error("Invalid response shape");
			return json.data;
		},
		initialPageParam: 1,
		getNextPageParam: (lastPage) =>
			lastPage.hasMore ? lastPage.page + 1 : undefined,
		enabled: Boolean(name && name.trim().length > 0),
	});
}
