...
import type { ProfileAnimeListItem } from "@/app/api/profile/types";

interface AnimeListTableProps {
	title: string;
	items: ProfileAnimeListItem[];
	showHeader?: boolean;
}
...
export function AnimeListTable({
	...
	const columns: ColumnDef<ProfileAnimeListItem>[] = [
		{
			id: "thumb",
			header: "",
			cell: ({ row }) => {
				const src = row.original.images ?? "";
				const href = `/anime/${row.original.animeId}`;
				return (
					<div className="h-12 w-9 overflow-hidden rounded-sm bg-muted">
						{src ? (
							...
							<Link
								...
								aria-label={row.original.title ?? "Anime"}
							/>
						)}
					</div>
				);
