...
program
	.command("insert-all")
	.description("Insert all characters from the Jikan API")
	.action(async () => {
		try {
			const fetcher = new CharactersFetcher();
			console.log("Inserting all characters from Jikan API...");
...
program
	.command("update-all")
	.description("Update all characters from the Jikan API")
	.action(async () => {
		try {
			const fetcher = new CharactersFetcher();
			console.log("Updating all characters from Jikan API...");
			console.log("This may take a while as it processes all paginated data.");
...
program
	...
	.description("Insert a single character by MAL ID")
	.action(async (id: string) => {
		try {
			...
			console.log(`Inserting character with ID: ${malId}`);
...
program
	...
	.description("Insert characters starting from ID until no more entries found")
	...
	.action(async (startId: string | undefined, options: { start?: string }) => {
		try {
			...
			console.log(`Inserting characters starting from ID: ${start}`);
...
program
	...
	.description("Insert characters between two MAL IDs (inclusive)")
	.action(async (startId: string, endId: string) => {
		try {
			...
			console.log(`Inserting characters between IDs: ${start} - ${end}`);
...
program
	...
	.description("Update a single character by MAL ID")
	.action(async (id: string) => {
		try {
			const fetcher = new CharactersFetcher();
			const malId = Number.parseInt(id, 10);
			...
			console.log(`Updating character with ID: ${malId}`);
...
program
	...
	.description("Update characters starting from ID until no more entries found")
	.option("-s, --start <id>", "Starting MAL ID", "1")
	.action(async (startId: string | undefined, options: { start?: string }) => {
		try {
			const fetcher = new CharactersFetcher();
			...
			console.log(`Updating characters starting from ID: ${start}`);
...
program
	...
	.description("Update characters between two MAL IDs (inclusive)")
	.action(async (startId: string, endId: string) => {
		try {
			const fetcher = new CharactersFetcher();
			const start = Number.parseInt(startId, 10);
			...
			console.log(`Updating characters between IDs: ${start} - ${end}`);
...
program
	...
	.description("Insert characters for a list of MAL IDs (space-separated)")
	.action(async (ids: string[]) => {
		try {
			...
			console.log(`Inserting characters for IDs: ${malIds.join(", ")}`);
...
program
	...
	.description("Update characters for a list of MAL IDs (space-separated)")
	.action(async (ids: string[]) => {
		try {
			const fetcher = new CharactersFetcher();
			const malIds = ids
			...
			console.log(`Updating characters for IDs: ${malIds.join(", ")}`);
